...
const Logo = () => (
  <svg width="24" height="24" viewBox="0 0 168 168" version="1.1" xmlns="http://www.w3.org/2000/svg">
...
const Header = ({ currentView }) => {
  ...
  return (
    <header className="bg-theme-light-bg-primary dark:bg-theme-dark-bg-primary border-b border-theme-light-border-primary dark:border-theme-dark-border-primary px-4 py-4 sticky top-0 z-10">
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-2">
          <div className="text-theme-light-text-primary dark:text-theme-dark-text-primary">
            <Logo />
          </div>
          <h1 className="text-xl font-semibold text-theme-light-text-primary dark:text-theme-dark-text-primary">
            {getTitle()}
          </h1>
        </div>
        <button
          onClick={toggleTheme}
          className="p-2 rounded-full hover:bg-theme-light-bg-secondary dark:hover:bg-theme-dark-bg-secondary transition-colors"
          aria-label={theme === 'light' ? 'Switch to dark mode' : 'Switch to light mode'}
        >
          {theme === 'light' ? (
            <MoonIcon className="h-5 w-5 text-theme-light-text-secondary dark:text-theme-dark-text-secondary" />
          ) : (
            <SunIcon className="h-5 w-5 text-theme-light-text-secondary dark:text-theme-dark-text-secondary" />
          )}
        </button>
      </div>
    </header>
